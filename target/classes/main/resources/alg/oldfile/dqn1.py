...
def main(rate, reward,iter,gamma, lr, table_name, cols, labels):
    ...
    train_env = Environment1(X_train, y_train, reward)
    train_env.rate = rate
    Q, total_losses, total_rewards = train_dqn(train_env,iter,gamma, lr, X_train.shape[1])
    # plot_loss_reward(total_losses, total_rewards, rate)
    # 保存
    ...
    # explainer = shap.Explainer(Q, X_train[0:100, :])
    # 将解释器对象保存到文件
    ...
    test_env = Environment1(X_test, y_test, reward)
    # test
    test_env.rate = rate
    pobs = test_env.reset()
    test_acts = []
    ...
    for _ in range(len(test_env.data)):
        ...
        # if pact == 1 and test_env.data_label[_] == 1:
        #     TP += 1
        # if pact == 0 and test_env.data_label[_] == 1:
        #     FN += 1
        # if pact == 1 and test_env.data_label[_] == 0:
        #     FP += 1
        # if pact == 0 and test_env.data_label[_] == 0:
        #     TN += 1
    ...
    # file_path = os.path.join(FIG_SAVE_DICTORY, f"DQN_{rate}_confusion_matrix.png")
    # plt.savefig(file_path, dpi=300, bbox_inches='tight')
